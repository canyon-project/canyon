...
### 查询参数
- **subject** (必填)：`commit | commits | pull | pulls | multiple-commits`
- **subjectID** (必填)：当 subject 是 commit/commits 时为 `sha`；当为 pull/pulls 时为 `pullNumber`；当为 multiple-commits 时为提交范围 `a...b`
...
- **mode** (可选)：`blockMerge | fileMerge | default`（pull/pulls/multiple-commits 时使用）
...
- 当 subject 为 `multiple-commits` 时，`subjectID` 形如 `a...b`，以 `b` 为基线进行范围聚合；
...
- **路径（Multiple Commits）**：`/api/provider/:provider/repo/:repoID/coverage/overview/multiple-commits/:commitRange`
...
- **pullNumber**（pull 路径）：合并请求编号
 - **commitRange**（multiple-commits 路径）：提交范围，格式为 `a...b`
...
## 纯报告页链接（Report Page URL）

- **用途**：直接进入前端报告页（可选携带查询参数以指定构建/报告来源）。
...
### 路由占位符
- `:provider`：`gitlab | github | ...`（页面路由用）
- `:org`：组织或命名空间，例如 `canyon-project`
- `:repo`：仓库名，例如 `canyon-demo`
- `:subject`：`commit | commits | pull | pulls | multiple-commits`
- `:subjectID`：
  - 当 subject 为 commit/commits：提交 `sha`
  - 当 subject 为 pull/pulls：`pullNumber`
  - 当 subject 为 multiple-commits：提交范围 `a...b`，其中以 `b` 作为基线
...
### 可选查询参数（用于后端数据选择）
- `provider`：后端使用的代码托管提供方（默认 `gitlab`）。
- `build_provider`：构建/CI 提供方（例如 `gitlab_runner`、`jenkins`）。
- `build_id`：构建 ID。
- `report_provider`：报告来源（如 `mpaas`、`flytest`、`person`）。
- `report_id`：报告 ID。
