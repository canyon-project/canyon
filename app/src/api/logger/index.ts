...
export function logger(data: LogMessage) {
  ...
  // hono 风格的日志行
  const logLine = `${colors.green}[Hono] ${pid}${colors.reset}  - ${timestamp}     ${config.color}${config.label}${colors.reset} ${colors.yellow}[${data.title}]${colors.reset} ${messageContent}`;
