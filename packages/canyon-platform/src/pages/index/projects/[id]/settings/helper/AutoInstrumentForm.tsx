...
const AutoInstrumentForm = ({onChange,defaultDataSource}) => {
    ...
    const columns = [
        ...
        },
        {
            ...
            render: (_,record) => <div>
                ...
                <a type="link" style={{color:'red'}} onClick={
                    ()=>{
                        let newDataSource = dataSource.filter(item=>item.filepath!==record.filepath)
  ...
  return <div>
      ...
      <Table key={'filepath'} pagination={false} bordered={true} size={'small'} dataSource={dataSource} columns={columns} />
      <Drawer title={`${mode==='create'?'添加':'编辑'} ${activeFilepath}`} width={'45%'} open={open} onClose={onClose}>
          <Form>

              <Form.Item label={'文件路径'}>
                    <Input disabled={mode==='update'} value={activeFilepath} onChange={(e)=>{
                  ...
                  <Form.Item label={'文件内容'}>
                      <div style={{
                          ...
                          <Editor
                              ...
                              value={activeContent} height={'70vh'} onChange={(v)=>{
                              setActiveContent(v||'')
                          }}/>
                      </div>
                  </Form.Item>
