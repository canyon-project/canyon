...
import { Breadcrumb, Button, Divider, Form, Input, Select, Space, Typography, Card, Descriptions, message } from 'antd';
...
const NewProjectPage = () => {
  ...
  const onCheck = async () => {
    ...
    try {
      ...
      const { data } = await axios.get(`/api/code/projects/${encoded}`, { params: { provider } });
  ...
  return (
    <div className={'p-3'}>
      <div className={'flex h-[48px] items-center justify-between px-[4px]'}>
        <Breadcrumb
          items={[
            { title: <Link to={'/index/projects'}>Project</Link> },
            { title: 'Create' },
          ]}
        />
        <Space>
      ...
      <Card title={'创建项目'} bordered={false}>
          ...
          {project ? (
            <Descriptions column={1} size={'small'}>
              ...
              <Descriptions.Item label='pathWithNamespace'>
                {project.path_with_namespace ?? project.pathWithNamespace ?? <Text type='secondary'>-</Text>}
              </Descriptions.Item>
        ...
        <Space style={{ marginTop: 16 }}>
          ...
          <Text type='secondary'>后端未提供创建接口，当前仅支持校验与查看。</Text>
        </Space>
      </Card>
    </div>
  );
