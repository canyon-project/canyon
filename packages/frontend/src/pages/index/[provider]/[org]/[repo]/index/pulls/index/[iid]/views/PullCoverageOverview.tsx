...
const PullCoverageOverview = () => {
  ...
  const { data: summary } = useRequest(
    () => {
      ...
      return axios
        .get(
          `/api/coverage/overview?subject=pull&subjectID=${pull.iid}&provider=gitlab&repoID=${repo.id}`
        )
        .then((res) => res.data);
    },
  ...
  const { data: fileMap } = useRequest(
    () => {
      ...
      return axios
        .get(
          `/api/coverage/summary/map?subject=pull&subjectID=${pull.iid}&provider=gitlab&repoID=${repo.id}`
        )
        .then((res) => res.data)
        .then((obj) => {
          if (!obj) return [];
      ...
      <Card size='small'>
        <Space align='center' className='w-full justify-between'>
          ...
          <div className='flex items-center gap-3'>
            ...
            <Button type='primary' size='small' onClick={handleOpenDetail}>
              查看
            </Button>
          </div>
        </Space>
