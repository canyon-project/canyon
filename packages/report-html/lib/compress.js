...
function compress(jsonstring) {
  debug(
    'Starting compression of JSON string (length: %d characters)',
    jsonstring.length,
  );

  const originalSize = Buffer.byteLength(jsonstring, 'utf8');
  debug(
    'Original data size: %d bytes (%.2f KB)',
    originalSize,
    originalSize / 1024,
  );
  ...
  debug(
    'Gzip compressed size: %d bytes (%.2f KB)',
    compressedSize,
    compressedSize / 1024,
  );
  ...
  debug(
    'Base64 encoded size: %d bytes (%.2f KB)',
    base64Size,
    base64Size / 1024,
  );
  ...
  debug(
    'Compression statistics - Gzip ratio: %s%%, Overall ratio: %s%%',
    compressionRatio,
    base64Ratio,
  );
