        ...
        Object.entries(coverageData).map(([key, value]) => [
            key,
            {
                ...
                b: Object.fromEntries(
                    Object.entries(value.b).map(([k, v]) => [k, { data: v }]),
                ),
            },
        ]),
    );
...
function deFormatCoverageDataToProto(deCoverageData) {
    // @ts-ignore
    return Object.fromEntries(
        Object.entries(deCoverageData).map(([key, value]) => [
            key,
            {
                // @ts-ignore
                ...value,
                // @ts-ignore
                b: Object.fromEntries(
                    Object.entries(value.b).map(([k, v]) => [k, v.data]),
                ),
            },
        ]),
    );
    ...
    return new Promise((resolve, reject) => {
        protobuf.load("proto/coverage.proto", function (err, root) {
            ...
            compress(protobufBuffer).then((zstdBuffer) => {
                ...
                console.log(
                    `zstd+protobuf压缩，压缩耗时: ${new Date().getTime() - time} ms`,
                );
                console.log(
                    `zstd+protobuf压缩，压缩前大小：${byteLength}b，压缩后大小：${Buffer.byteLength(zstdBuffer)}b`,
                );
                console.log(
                    `zstd+protobuf压缩，压缩率：${((100 * (byteLength - Buffer.byteLength(zstdBuffer))) / byteLength).toFixed(2)}%`,
                );
                resolve(zstdBuffer);
            });
        });
...
export async function compressCoverageDataByZstd(str) {
    ...
    console.log(
        `zstd直接压缩，压缩前大小：${str.length}b，压缩后大小：${Buffer.byteLength(compressed)}b`,
    );
    console.log(
        `zstd直接压缩，压缩率：${((100 * (byteLength - Buffer.byteLength(compressed))) / byteLength).toFixed(2)}%`,
    );
    return compressed;
}
