...
export class CodeService {
  ...
  private async resolveGithubOwnerRepoByID(idOrSlug: string, base: string, token: string) {
    // 如果是 owner/repo 直接返回
    ...
    if (/^[0-9]+$/.test(idOrSlug)) {
      ...
      const data = resp.data as { full_name?: string; owner?: { login?: string }; name?: string };
  ...
  private async getFileContentFromGithub({
    ...
    if (!ref && pullNumber) {
      const { owner, repo } = await this.resolveGithubOwnerRepoByID(repoID, base, token);
  ...
  async getProjectByPathGithub(path: string, provider?: string) {
    ...
    if (/^[0-9]+$/.test(path)) {
      ...
      if (!owner || !repo) {
        throw new BadRequestException('GitHub 项目路径需为 owner/repo 或数字 ID');
      }
