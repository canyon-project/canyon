export function tupleToMap(t: any): Record<string, number> {
  if (!Array.isArray(t) || t.length !== 2) return {} as Record<string, number>;
  const [keys, values] = t as [any[], any[]];
  const out: Record<string, number> = {};
  const len = Math.min(keys?.length ?? 0, values?.length ?? 0);
  for (let i = 0; i < len; i++) {
    out[String(keys[i])] = Number(values[i] ?? 0);
  }
  return out;
}
...
export function posToOffset(line: number, col: number, starts: number[]): number {
  const idx = Math.max(0, Math.min(starts.length - 1, line - 1));
  return starts[idx] + col;
}

export function sliceByStmtLoc(content: string, info: any, starts: number[]): [string, number] {
  const start = posToOffset(Number(info.start_line || info.startLine || 0), Number(info.start_column || info.startColumn || 0), starts);
  const end = posToOffset(Number(info.end_line || info.endLine || 0), Number(info.end_column || info.endColumn || 0), starts);
  const s = Math.max(0, Math.min(start, content.length));
  const e = Math.max(s, Math.min(end, content.length));
  return [content.slice(s, e), s];
}

export function buildStmtHashGroups(content: string, stmtMap: Record<string, any>): Map<string, Array<{ id: string; pos: number }>> {
...
export function sliceByFnLoc(content: string, info: any, starts: number[]): [string, number] {
  const endPos = info.end_pos || info.endPos || [0, 0, 0, 0];
  const start = posToOffset(Number(endPos[0] || 0), Number(endPos[1] || 0), starts);
  const end = posToOffset(Number(endPos[2] || 0), Number(endPos[3] || 0), starts);
  const s = Math.max(0, Math.min(start, content.length));
  const e = Math.max(s, Math.min(end, content.length));
  return [content.slice(s, e), s];
}

export function buildFnHashGroups(content: string, fnMap: Record<string, any>): Map<string, Array<{ id: string; pos: number }>> {
  const groups = new Map<string, Array<{ id: string; pos: number }>>();
...
export function mergeStatementHitsByBlock(
  baseContent: string,
  baseStmtMap: Record<string, any>,
  otherContent: string,
  otherStmtMap: Record<string, any>,
...
export function mergeFunctionHitsByBlock(
  baseContent: string,
  baseFnMap: Record<string, any>,
  otherContent: string,
  otherFnMap: Record<string, any>,
  otherHits: Record<string, number>
): Record<string, number> {
  const result: Record<string, number> = {};
