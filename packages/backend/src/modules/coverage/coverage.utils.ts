...
export function sliceByStmtLoc(
  ...
  const start = posToOffset(
    Number(info.start_line || info.startLine || 0),
    Number(info.start_column || info.startColumn || 0),
    starts,
  )
  const end = posToOffset(
    Number(info.end_line || info.endLine || 0),
    Number(info.end_column || info.endColumn || 0),
    starts,
  )
...
export function buildStmtHashGroups(
  ...
  for (const [h, arr] of groups)
    arr.sort((a, b) => a.pos - b.pos), groups.set(h, arr)
  return groups
}
