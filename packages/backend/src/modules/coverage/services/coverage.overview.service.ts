...
export class CoverageOverviewService {
  ...
  async getOverview(q: {
    ...
    const coverages = await this.covRepo.find(covWhere, {
      fields: [
        'id',
        'instrumentCwd',
        'buildProvider',
    ...
    for (const r of relationsAll) {
      ...
      coverageMapWithFilePath.push({
        fullFilePath: r.fullFilePath,
        hash: r.coverageMapHashID,
        S,
      });
    }
    ...
    const deduplicatedBuildGroupList: Array<{
      buildProvider: string;
      buildID: string;
    }> = [];
    for (const g of buildGroupList) {
      ...
      if (!dedupSet.has(k)) {
        dedupSet.add(k);
        deduplicatedBuildGroupList.push(g);
      }
    }

    // 拉取测试用例信息列表（仅 mpaas/flytest，按 (reportProvider, reportID) 去重并并发请求）
    const testCaseInfoList = await this._getTestCaseInfoList(coverages);
  ...
  private _calcCoverageSummary(
    coverageHit: Array<{ fullFilePath: string; s: unknown }>,
  ...
  private _buildAutoMode(
    coverageList: Array<{ reportProvider?: string | null }>,
    coverageHitData: Array<{
      coverageID: string;
      fullFilePath: string;
      s: unknown;
    ...
    const hits = this._filterCoverageHit(
      autoCoverages as Array<{ id: string }>,
      coverageHitData,
    );
    const paths = this._extractPathsFromHits(hits);
    const maps = this._filterCoverageMapWithFilePathByPaths(
      paths,
      coverageMapWithFilePath,
    );
    const summary = this._calcCoverageSummary(hits, maps);
    const caseList = this._buildCaseList(
      autoCoverages as Array<{
        id: string;
        repoID: string;
        sha: string;
  ...
  private _buildManualMode(
    ...
    const hits = this._filterCoverageHit(
      manualCoverages as Array<{ id: string }>,
      coverageHitData,
    );
    const paths = this._extractPathsFromHits(hits);
    const maps = this._filterCoverageMapWithFilePathByPaths(
  ...
  private _buildCaseList(
    ...
    for (const c of coverageList) {
      const singleHits = this._filterCoverageHit(
        [{ id: c.id }],
        coverageHitData,
      );
      ...
      const testCaseInfo = this._getTestCaseInfo(
        testCaseInfoList,
        c.reportID ?? undefined,
        c.reportProvider ?? undefined,
      );
      ...
      for (const [k, v] of Object.entries(testCaseInfo || {})) item[k] = v;
      out.push(item);
    }
    return out;
  }

  private _getTestCaseInfo(
  ...
  private _buildResultList(
    deduplicatedBuildGroupList: Array<{
      buildProvider: string;
      buildID: string;
    }>,
    coverageList: Array<{
    ...
    for (const group of deduplicatedBuildGroupList) {
      ...
      const current = coverageList.filter(
        (c) => c.buildProvider === buildProvider && c.buildID === buildID,
      );
      const hits = this._filterCoverageHit(
      ...
      const modeList = [
        this._buildAutoMode(
          current as Array<{ reportProvider?: string | null }>,
          coverageHitData,
          coverageMapWithFilePath,
          testCaseInfoList,
        ),
        this._buildManualMode(
          current as Array<{ reportProvider?: string | null }>,
          coverageHitData,
          coverageMapWithFilePath,
          testCaseInfoList,
  ...
  private async _getTestCaseInfoList(
    coverageList: Array<{
      reportProvider?: string | null;
      reportID?: string | null;
    }>,
  ...
  private async _fetchExternalTestCaseInfo(
    base: string,
    reportProvider: string,
    reportID: string,
