...
export class CoverageService {
  ...
  async getSummaryMap(q: any) {
    ...
    if (subject === 'commit' || subject === 'commits') {
      ...
      const qb: any = { provider, repoId: repoID, sha: subjectID } as any;
      ...
      for (const r of rows || []) {
        ...
        const totalBranches = structure
          ? Object.values(structure.branchMap || {}).reduce((a: number, v: any) => a + (Array.isArray((v as any).paths) ? (v as any).paths.length : 0), 0)
    ...
    if (subject === 'pull' || subject === 'pulls') {
      ...
      const mrResp = await axios.get(`${baseUrl}/api/v4/projects/${projectId}/merge_requests/${subjectID}`, { headers });
      ...
      const commitsResp = await axios.get(`${baseUrl}/api/v4/projects/${projectId}/merge_requests/${subjectID}/commits`, { headers });
      ...
      const allHashes = Array.from(new Set(
        Array.from(aggregated.keys()).map((absPath) => covPathToHash.get(`${baselineCovID}|${absPath}`)).filter((v): v is string => Boolean(v))
      ));
      const hashToMap = await this.fetchCoverageMapsFromClickHouse(allHashes);

      const out: Record<string, any> = {};
      for (const [absPath, agg] of aggregated) {
        const rel = trimInstrumentCwd(absPath, baselineCwd);
        const hash = covPathToHash.get(`${baselineCovID}|${absPath}`);
        const structure = hash ? hashToMap.get(hash) : undefined;
