...
func (s *CoverageService) getCoverageSummaryMapFastInternal(query dto.CoverageQueryDto) (map[string]interface{}, error) {
	...
	for mapRows.Next() {
		...
		if err := mapRows.Scan(&hash, &sKeys); err != nil {
			return nil, fmt.Errorf("扫描coverage_map数据失败: %w", err)
		}
		perHashSizes[hash] = mapSizes{SKeys: sKeys}
	}
	...
	for _, r := range relations {
		...
		// totals（仅 statements）；lines 等同 statements；functions/branches 复用 statements 覆盖率
		...
		summary := map[string]interface{}{
			...
			"functions":  map[string]interface{}{"total": totalS, "covered": coveredS, "skipped": 0, "pct": pct(coveredS, totalS)},
			"branches":   map[string]interface{}{"total": totalS, "covered": coveredS, "skipped": 0, "pct": pct(coveredS, totalS)},
