...
export const remapCoverageWithInstrumentCwd = async (
  noReMap: any,
  ...
  const reMapedCov = await remapCoverageByOld(noReMap);

  const obj2: any = {};
  for (const coverageKey in reMapedCov) {
    const newKey = coverageKey.replace(instrumentCwd + '/', '');
    // @ts-ignore
    const oldPath = reMapedCov[coverageKey].oldPath.replace(
      instrumentCwd + '/',
      '',
    );
    obj2[newKey] = {
