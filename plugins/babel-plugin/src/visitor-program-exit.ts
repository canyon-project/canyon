...
export function visitorProgramExit(
  ...
  try {
    if (
      ...
      enrichStatementMapWithHash(
        initialCoverageData.statementMap as Parameters<typeof enrichStatementMapWithHash>[0],
        originalCode,
      );
    }
  } catch (_error) {
  ...
  if (sourceCode.includes("coverageData")) {
    ...
    programPath.traverse({
      VariableDeclarator(variablePath: NodePath<BabelTypes.VariableDeclarator>) {
        ...
        if (
          ...
          const validProperties = objectExpression.properties.filter(
            (property): property is BabelTypes.ObjectProperty | BabelTypes.ObjectMethod =>
              types.isObjectProperty(property) || types.isObjectMethod(property),
          );
