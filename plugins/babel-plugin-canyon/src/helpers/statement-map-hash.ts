...
type FunctionLoc = {
  name?: string;
  decl?: { start: Position; end: Position };
...
export function enrichStatementMapWithHash(
  ...
  const isValidPos = (p?: Position) =>
    p && typeof p.line === 'number' && typeof p.column === 'number';
...
export function enrichFnMapWithHash(
  ...
  Object.keys(fnMap).forEach((key) => {
    ...
    const hasLoc =
      info &&
      info.loc &&
      isValidPos(info.loc.start) &&
      isValidPos(info.loc.end);
    const hasDecl =
      info &&
      info.decl &&
      isValidPos(info.decl.start) &&
      isValidPos(info.decl.end);
    ...
    if (hasLoc && hasDecl) {
      start = earlierOf(
        info.decl.start as Position,
        info.loc.start as Position,
      );
