...
- Core entities: Coverage Map, Coverage Hit, Scene, Build
...
A Build represents a **build/execution instance**. It identifies:

- Repository (provider + repoID)
- Commit SHA
- Build target
- Optional: Pipeline ID, Job ID

**Key Properties:**

- Unique per build instance
- Used for build-level aggregation
- Identified by `buildHash`
...
| `buildHash` | Build instance identity | Build, Hit, Map |
