...
async function generateLLMDocs() {
  ...
  const optionals = sortedFiles.map(
    (file) => `- [${capitalizeDelimiter(extractLabel(file))}](${DOCS_BASE_URL}${toDocPath(file)})`,
  );
  ...
  const smallFiles = sortedFiles.filter((file) => !smallExclude.some((ex) => file.startsWith(ex)));
...
async function generateContent(files: string[], baseDir: string, header: string): Promise<string> {
